...
scale _s_. Test = untouched 551-image split, evaluated once per model.
...
| v7b       | [`models/yolo26-rd-v7b.yaml`](models/yolo26-rd-v7b.yaml)             | v7 with the P2 _detection_ level removed (module-free control for v8b)                                         | —         | —          |
...
| v8e       | [`models/yolo26-rd-v8e.yaml`](models/yolo26-rd-v8e.yaml)             | v8b + v8c combined — _interferes; worse than either parent_                                                    | 0.766     | 0.703      |
| v8b-dfl   | [`models/yolo26-rd-v8b-dfl.yaml`](models/yolo26-rd-v8b-dfl.yaml)     | v8b with distribution box regression restored (`reg_max` 16) — _no gain measured_                              | 0.759     | —          |
| v8b-strip | [`models/yolo26-rd-v8b-strip.yaml`](models/yolo26-rd-v8b-strip.yaml) | v8b + zero-init directional **strip context** (dw 1×9 + 9×1) at head-P3 — _under evaluation_                   | —         | —          |
...
- **v8b / v8** — checkpoints are _not_ interchangeable: v8 has four detection scales (P2–P5),
...
- **v8b-strip** — starts _exactly_ equal to v8b (zero-initialized gate), so short runs that show
...
model at 800/960 _reduces_ mAP50 (0.787 → 0.758 → 0.701). Note that test-time augmentation
